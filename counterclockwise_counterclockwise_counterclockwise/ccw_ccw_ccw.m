...
bus_location = [0 7 15]; % Numbers are distance from first stop
...
for t = 1:720
    ...
    bus_location(1,2) = bus_location(1,2) - 1;
    bus_location(1,3) = bus_location(1,3) - 1;
...
subplot(1,1,1)
    ...
    hold off
    legend({'bus 1', 'bus 2', 'bus 3'},'location','southeast')
    title('Bus passengers')

    
saveas(gcf, ['ccw_ccw_ccw_bus_' num2str(seed) '.png'])
...
saveas(gcf, ['ccw_ccw_ccw_stop_' num2str(seed) '.png'])


%% save

save(['ccw_ccw_ccw_' num2str(seed)])
